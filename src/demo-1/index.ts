...
import { initGPU, writeBuffer } from '../utils/index';
async function main() {
  const device = await initGPU();
  const firstMatrix = [2 /* rows */, 4 /* columns */, 1, 2, 3, 4, 5, 6, 7, 8];
  const secondMatrix = [4 /* rows */, 2 /* columns */, 1, 2, 3, 4, 5, 6, 7, 8];
  const gpuBufferFirstMatrix = writeBuffer({
    device,
    array: firstMatrix,
    descriptor: {
  ...
  const gpuBufferSecondMatrix = writeBuffer({
    device,
    array: secondMatrix,
    descriptor: {
      usage: GPUBufferUsage.STORAGE
